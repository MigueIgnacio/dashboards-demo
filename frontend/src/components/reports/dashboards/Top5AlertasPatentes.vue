<template>
    <div class="top-speeding-container">
        <!-- Contenedor del título y botón con estilos inline -->
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h2 class="titulo">
                {{ isDriverView ? "TOP 5 CONDUCTORES POR CANTIDAD DE EXCESOS" : "TOP 5 PATENTES POR CANTIDAD DE EXCESOS"
                }}
            </h2>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>{{ isDriverView ? 'Cantidad de Alertas' : 'Cantidad de Alertas' }}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr v-else v-for="(item, index) in filteredData" :key="index" @click="sendData(item)"
                        ...
                        <td>
                            {{ isDriverView ? item.alert_count : item.alert_count }}
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    computed: {
        filteredData() {
            return this.topSpeedingData.filter(item =>
                this.isDriverView
                    ? item.driver_name && item.driver_rut && item.alert_count !== undefined
                    : item.vehicle_plate && item.vehicle_code && item.alert_count !== undefined
            );
        }
    },
    mounted() {
    ...
    methods: {
        ...
        exportToExcel() {
            ...
            const endpoint = this.isDriverView
                ? "/dashboard/getExcelTopSpeedingDrivers"
                : "/dashboard/getExcelTopSpeedingVehicles";

            const filename = this.isDriverView
                ? "top_5_conductores_con_mas_excesos.xlsx"
                : "top_5_vehiculos_con_mas_excesos.xlsx";
        ...
        fetchTopSpeedingData() {
            if (this.isValidDateRange()) {
                ...
                const endpoint = this.isDriverView
                    ? "/reports/dashboard-excess-alert-speed/topDriversByAlerts"
                    : "/reports/dashboard-excess-alert-speed/getTopVehiclesByAlerts";

                axios.get(endpoint, { params })
                    .then((response) => {
                        const raw = response.data || [];
                        this.topSpeedingData = raw.map(i => ({
                            ...i,
                            alert_count: Number(i.alert_count ?? i.alerts) || 0
                        }));
                        this.isLoading = false;
                    })
                    .catch((error) => {
